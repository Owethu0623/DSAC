...
export const DsacFeatureSideView: React.FC<DsacFeatureSideViewProps> = ({
  ...
  const [entityFilter, setEntityFilter] = useState<'ALL' | 'PUBLIC_ENTITY' | 'NPO'>('ALL');
  ...
  const [selectedReportQuarter, setSelectedReportQuarter] = useState<'Q1' | 'Q2' | 'Q3'>('Q2');
  ...
  // KPIs for this entity
  const entityKpis = useMemo(() => {
    if (!currentEntity) return [];
    return store.kpis.filter(k => k.entityId === currentEntity.id);
  }, [currentEntity]);
        ...
        <div className="p-3 bg-white border-b border-slate-200 space-y-2 shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div className="flex items-center gap-1">
              ...
              <div className="inline-flex rounded-lg border border-slate-200 bg-slate-100 p-0.5 text-[10px] font-bold">
                ...
                <button
                  onClick={() => setEntityFilter('PUBLIC_ENTITY')}
                  className={`px-2 py-0.5 rounded cursor-pointer ${
          ...
          <div className="relative flex items-center gap-2">
            <div className="relative flex-1">
              ...
              >
                <optgroup label="Statutory Public Entities (26)">
                  {entities.filter(e => e.type === 'PUBLIC_ENTITY').map(e => (
                    <option key={e.id} value={e.id}>
                      {e.shortCode} - {e.name} ({e.cluster})
                    </option>
                  ))}
                </optgroup>
                <optgroup label="Subsidized Cultural NPOs (6)">
                  {entities.filter(e => e.type === 'NPO').map(e => (
                    <option key={e.id} value={e.id}>
                      {e.shortCode} - {e.name} ({e.cluster})
                    </option>
                  ))}
            ...
            {onOpenWorkspace && currentEntity && (
              <button
                ...
                className="px-3 py-2 bg-emerald-800 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-xs shrink-0 cursor-pointer"
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="p-3.5 bg-white border border-slate-200 rounded-xl space-y-3">
                <div className="font-bold text-slate-800 text-[11px] uppercase tracking-wider flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-emerald-700 font-semibold">{entityKpis.length} Tracked</span>
                </div>
              ...
              <div className="p-3.5 bg-white border border-slate-200 rounded-xl space-y-2">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-slate-50 border border-slate-200">
                    <div>
                      <div className="font-bold text-slate-800">Tranche 1 (Q1 - 25%)</div>
                      <div className="text-[10px] text-slate-500">Paid 15 April 2024 • Section 38 Verified</div>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-2 rounded-lg bg-slate-50 border border-slate-200">
                    <div>
                      <div className="font-bold text-slate-800">Tranche 2 (Q2 - 25%)</div>
                      <div className="text-[10px] text-slate-500">Paid 15 July 2024 • Q1 Performance Endorsed</div>
                    </div>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-100 text-emerald-800">Disbursed</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-2 rounded-lg bg-slate-50 border border-slate-200">
                    <div>
                      <div className="font-bold text-slate-800">Tranche 3 (Q3 - 25%)</div>
                      <div className="text-[10px] text-slate-500">Paid 15 October 2024 • Q2 Reports Cleared</div>
                    </div>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-100 text-emerald-800">Disbursed</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-2 rounded-lg bg-blue-50/50 border border-blue-200">
                    <div>
                      <div className="font-bold text-blue-900">Tranche 4 (Q4 - 25%)</div>
                      <div className="text-[10px] text-slate-500">Pending Q3 Statutory Review &amp; AFS Baseline</div>
                    </div>
                    <button
